...
root = Tk()
root.title("Advanced calculator by Yash Gaur")
...
lblDisplay=Label(calc, text="Advanced Calculator ", font=('arial', 30, 'bold'), justify =CENTER)
lblDisplay.grid(row=0, column=4, columnspan=4)

lblDisplay=Label(calc, text="YRS CODING HUB", font=('arial', 30, 'bold'), justify =CENTER)
lblDisplay.grid(row=6, column=0, columnspan=4)

def iExit():
    iExit = tkinter.messagebox.askyesno("Advanced Calculator", "Confirm if you want to exit")
    if iExit>0:
        root.destroy()
        return
